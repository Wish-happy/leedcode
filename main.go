...
const (
	Repository = "https://github.com/Wish-happy/leetcode"
...
var problemStubs = map[string]*Stub{
	"default": {"problem.stub", "index.ts"},
	"tree":    {"tree.stub", "index.ts"},
	"link":    {"link.stub", "index.ts"},
	"sql":     {"sql.stub", "1.sql"},
}
...
func buildReadme() {
	...
	for _, index := range solutionOrder {
		...
		for k, problem := range problems {
			...
			class = append(class, fmt.Sprintf("\n| %s | [%-32s](https://leetcode-cn.com/problems/%s) | %s | %s | [TS](%s)| %s",
				questionId,
				questionTitle,
				questionTitleSlug,
				questionAcceptance,
